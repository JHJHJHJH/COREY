...
export class BrowserBridgeRegistry {
    ...
    this.server.on("connection", (socket: WebSocket, request: IncomingMessage) => {
      ...
      socket.on("message", (data: RawData) => {
        ...
        if (message.type === "state") {
          ...
          if (session && session.socket === socket && message.descriptor.sessionId === claims.sessionId) {
  ...
  attach(server: HttpServer) {
    server.on("upgrade", (request, socket, head) => {
      ...
      const claims = verifyCoreyMcpBridgeToken(url.searchParams.get("token") ?? "", this.secret);
      if (!claims) {
  ...
  disconnectAll(reason = "MCP access disabled") {
    for (const session of this.sessions.values()) {
