...
export function encodeMcpCursor(payload: CursorPayload) {
  return Buffer.from(JSON.stringify(payload), "utf8").toString("base64url");
}

export function decodeMcpCursor(cursor: string | undefined, revision: string) {
  if (!cursor) return 0;
...
function findColumn(data: ViewerDataTableData, field: CoreyMcpFieldRef) {
  ...
  return (
    data.columns.find(
      (column) =>
        column.key.toLowerCase() === wanted ||
        (column.binding ? fieldKey(column.binding).toLowerCase() === wanted : false),
    ) ?? null
  );
}

export function globalIdForRow(row: ViewerDataTableData["rows"][number]) {
...
function isWithinAny(
  globalId: string | null,
  wantedAncestors: Set<string>,
  spatial: CoreyMcpSpatialIndex | null | undefined,
) {
  if (!globalId || wantedAncestors.size === 0) return wantedAncestors.size === 0;
  if (!spatial) return false;
  let current: string | null = globalId;
  const visited = new Set<string>();
  while (current && !visited.has(current)) {
    if (wantedAncestors.has(current)) return true;
    visited.add(current);
    current = spatial.nodes[current]?.parentGlobalId ?? null;
  }
  return false;
}
...
export function queryMcpElements(input: {
  ...
  spatial?: CoreyMcpSpatialIndex | null;
  ...
  const matched = data.rows.filter((row) => {
    ...
    if (!isWithinAny(globalIdForRow(row), wantedAncestors, input.spatial)) return false;
  ...
  return {
    ...
    items,
    nextCursor:
      nextOffset < matched.length ? encodeMcpCursor({ revision, offset: nextOffset }) : null,
  };
}
...
export function queryValidationIssues(input: {
  ...
  const matches = (input.result?.results ?? []).filter((entry) => {
    ...
    if (clauseIds.size > 0 && !entry.failedClauses.some((clause) => clauseIds.has(clause.clauseId))) {
      return false;
    }
    const row = rowsByLocalId.get(entry.localId);
    return ifcTypes.size === 0 || ifcTypes.has((row?.ifcType ?? "").toUpperCase());
  });
  const items = matches.slice(offset, offset + limit).map((entry) => {
    ...
    return {
      ...
      name: row?.cells.name?.text ?? row?.selection.label ?? null,
      result: entry.result,
