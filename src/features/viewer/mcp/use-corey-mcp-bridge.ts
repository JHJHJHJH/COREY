...
type ConnectionState = "disabled" | "connecting" | "connected" | "error";

type CommandParams<Method extends CoreyMcpBridgeCommand["method"]> =
  Extract<CoreyMcpBridgeCommand, { method: Method }>["params"];

type CommandHandlers = {
  get_model_summary: (params: CommandParams<"get_model_summary">) => Promise<unknown> | unknown;
  query_elements: (params: CommandParams<"query_elements">) => Promise<unknown> | unknown;
  get_elements: (params: CommandParams<"get_elements">) => Promise<unknown> | unknown;
  get_validation_summary: (
    params: CommandParams<"get_validation_summary">,
  ) => Promise<unknown> | unknown;
  query_validation_issues: (
    params: CommandParams<"query_validation_issues">,
  ) => Promise<unknown> | unknown;
  apply_draft_edits: (params: CommandParams<"apply_draft_edits">) => Promise<unknown> | unknown;
  update_view: (params: CommandParams<"update_view">) => Promise<unknown> | unknown;
  open_stored_model: (params: CommandParams<"open_stored_model">) => Promise<unknown> | unknown;
  refresh_draft: (params: CommandParams<"refresh_draft">) => Promise<unknown> | unknown;
};
...
export function useCoreyMcpBridge(input: {
  ...
  const [state, setState] = useState<ConnectionState>("disabled");
  const [broadcastRevision, setBroadcastRevision] = useState(0);
  ...
  useEffect(() => {
    ...
    channel.addEventListener("message", () => {
      setBroadcastRevision((revision) => revision + 1);
    });
          ...
          void (async () => {
            ...
            try {
              ...
              if (socket.readyState === WebSocket.OPEN) {
                socket.send(
                  JSON.stringify({ type: "result", requestId: message.requestId, ok: true, value }),
                );
              }
            } catch (error) {
              if (socket.readyState === WebSocket.OPEN) {
  ...
  }, [broadcastRevision, input.configurationRevision]);
