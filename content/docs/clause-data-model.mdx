...
| `type` | `kind`, `expectedType` | The native scalar kind is exactly `string`, `number`, or `boolean`. |

Boolean checks understand common IFC/table values such as `true`, `false`, `1`,
`0`, `yes`, `no`, `y`, `n`, `.t.`, `.f.`, `t`, and `f`.

Type checks do not coerce display text. For example, `"42"` is a string rather
than a number, and `"true"` is a string rather than a boolean.
