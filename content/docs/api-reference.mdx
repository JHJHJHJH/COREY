...
- `GET /api/rule-templates`: list available validation templates, each with a
  `kind` of `config` (a whole clause set) or `clause` (a single clause)
- `POST /api/rule-templates`: save a template from a validation config
