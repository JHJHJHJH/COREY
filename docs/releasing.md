...
4. Merge the verified release candidate into `release`. The merge runs the
   ...
   example, version `1.0.0` publishes:

   ```text
   ghcr.io/jhjhjhjh/corey:1.0.0
   ghcr.io/jhjhjhjh/corey-mcp:1.0.0
   ```
...
5. Verify the published manifests:
   ...
   ```bash
   docker buildx imagetools inspect ghcr.io/jhjhjhjh/corey:1.0.0
   docker buildx imagetools inspect ghcr.io/jhjhjhjh/corey-mcp:1.0.0
   ```
